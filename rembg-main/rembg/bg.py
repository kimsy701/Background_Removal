...
from torch._C import _set_cublas_allow_fp16_reduced_precision_reduction
...
def remove(
    ...
    **kwargs: Optional[Any]
    ...
    if isinstance(data, PILImage):
        ...
        img = Image.fromarray(data) #numpy to PIL image
    else:
    ...
    print("image shape1",img.shape) #배경제거할 이미지 들어오면 해당 글 출력
    img = fix_image_orientation(img)
    print("image shape2",img.shape) #배경제거할 이미지 수정되면 해당 글 출력

    if session is None:
        #session = new_session("u2net", *args, **kwargs)
        session = new_session("u2net_custom", *args, **kwargs) #세션 생성
        print("session:", session) #세션 생성되면 해당 글 출력
